<template>
  <Demo-block title="Input 输入框" subtitle="通过鼠标或键盘输入内容，是最基础的表单域的包装。" :README="README">
    ...
    <Demo-row
      ...
      :code="example5"
    >
      <j-input type="textarea" placeholder="请输入内容"/>
    </Demo-row>
  </Demo-block>
</template>
...
<script>
  ...
  export default {
    ...
    computed: {
      example1() {
        return `<j-input placeholder="请输入内容" v-model="inputValue" />`;
      },
      example2() {
        return `<j-input disabled placeholder="请输入内容" />`;
      },
      example3() {
        return `<j-input placeholder="请输入内容" prefix-icon="search" />
<j-input placeholder="请输入内容" suffix-icon="date_range" />`;
      },
      example4() {
        return `<j-input placeholder="请输入内容">
    <template slot="prepend">
        <div class="prepend-part c-size-s">Http://</div>
    </template>
    ...
    <template slot="append">
      ...
      example5() {
        return `<j-input type="textarea" placeholder="请输入内容"/>`;
      }
    },
    methods: {
      handleClick() {
