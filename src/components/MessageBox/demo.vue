<template>
  ...
  >
    <Demo-row title="消息提示" description="当用户进行操作时会被触发，该对话框中断用户操作，直到用户确认知晓后才可关闭" :code="example1">
      <j-button class="demo-message-btn"  @click.stop="handleClick('alert')">点击打开</j-button>
    </Demo-row>

    <Demo-row title="自定义" description="可自定义配置不同内容" :code="example2">
      <j-button class="demo-message-btn" @click.stop="handleClick('custom-content')">点击打开</j-button>
    </Demo-row>

    <Demo-row title="区分取消与关闭" description="有些场景下，点击取消按钮与点击关闭按钮有着不同的含义" :code="example3">
      <j-button class="demo-message-btn" @click.stop="handleClick('custom')">点击打开</j-button>
    </Demo-row>
  </Demo-block>
</template>
